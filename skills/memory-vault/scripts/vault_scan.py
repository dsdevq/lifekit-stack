...
WIKI_PREFIXES = ("domains/", "projects/", "system/", "concepts/", "entities/", "syntheses/", "incidents/")
WIKI_ROOTS = {"index.md", "log.md", "PLAN.md", "README.md", "AGENTS.md", "WIKI.md", "inbox.md"}
LINK_RE = re.compile(r"\[\[([^\]|#]+)(?:#[^\]|]*)?(?:\|[^\]]*)?\]\]")


def find_vault(cli):
    for c in (cli, os.environ.get("MEMORY_VAULT"), os.path.expanduser("~/memory"), "/srv/memory"):
...
def read_allowlist(vault):
    m = re.search(r"```vault-structure\n(.*?)```", open(os.path.join(vault, "README.md")).read(), re.S)
    if not m:
        return None
    return {ln.strip().rstrip("/") for ln in m.group(1).splitlines() if ln.strip() and not ln.strip().startswith("#")}
...
def main():
    ...
    if allow is None:
        findings.append("structure: README has no ```vault-structure block — freeze not machine-readable")
    else:
        for entry in sorted(os.listdir(vault)):
            if entry in SKIP_DIRS or entry in {".gitignore", ".DS_Store"}:
                continue
            if entry.rstrip("/") not in allow:
                findings.append(f"structure: top-level '{entry}' not in allowlist — needs a graded proposal (contract: structure freeze)")

    # -- index pages + links
    pages, paths = {}, set()
    for rel in walk_md(vault):
        paths.add(rel.lower()); paths.add(rel[:-3].lower())
    ...
    for rel in walk_md(vault):
        ...
        if text.startswith("---"):
            m = re.search(r"(?m)^aliases: \[([^\]]*)\]", text.split("---", 2)[1] if "---" in text[3:] else "")
            if m:
    ...
    for rel in walk_md(vault):
        ...
        text = open(os.path.join(vault, rel), encoding="utf-8", errors="replace").read()
        if is_wiki and rel not in WIKI_ROOTS and "index" not in os.path.basename(rel).lower():
            if not text.startswith("---") and "/tasks/" not in rel and "/runs/" not in rel:
                no_fm.append(rel)
        ...
        for m in LINK_RE.finditer(scrub):
            t = m.group(1).strip(); key = t.lower(); base = key.split("/")[-1]
    ...
    for t, c in broken.most_common():
        if c >= 3:
            findings.append(f"broken-link: [[{t}]] referenced {c}x — contract says write the page or de-link")
    for rel in sorted(pages.values()):
        if not rel.startswith(WIKI_PREFIXES) or "index" in os.path.basename(rel).lower():
            continue
        if "/tasks/" in rel or "/runs/" in rel:
            continue
        if inbound.get(rel, 0) == 0:
            findings.append(f"orphan: {rel} has no inbound [[links]] — link from an INDEX or related page")
    findings += [f"missing-frontmatter: {r}" for r in no_fm]
    findings += [f"legacy-field: {r} still uses last_updated (contract: updatedAt)" for r in legacy]
    ...
    if os.path.isdir(proj):
        for d in sorted(os.listdir(proj)):
            ...
            if not os.path.isdir(dd):
                continue
            missing = [f for f in ("plan.md", "log.md", "journal.md") if not os.path.exists(os.path.join(dd, f))]
            if missing:
                findings.append(f"project-incomplete: projects/{d}/ missing {', '.join(missing)}")
