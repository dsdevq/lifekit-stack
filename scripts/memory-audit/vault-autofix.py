...
fixes = []
def log(action, path, detail): fixes.append({"action": action, "path": os.path.relpath(path, VAULT), "detail": detail})
...
def is_frozen(f):
    r = rel(f); return r.startswith(FROZEN_PREFIX) or any(s in "/" + r for s in FROZEN_SEG)
def skip(f): return is_frozen(f) or os.path.basename(f) in OPT or os.path.basename(f) in INDEXISH

def split_fm(s):
    if not s.startswith("---"): return None, None, s
    parts = s.split("---", 2)
    if len(parts) < 3: return None, None, s
...
def safe_write(path, text):
    if not valid_fm(text):
        log("skipped-unsafe", path, "auto-fix would produce malformed frontmatter — left unchanged")
        return False
...
def quote_bad_scalars(head):
    ...
    for ln in head.split("\n"):  # split (not splitlines) preserves the trailing newline before ---
        ...
        if m:
            ...
            if val[:1] not in ("'", '"', "|", ">", "[", "{", "&", "*", "#") and re.search(r"\S:\s", val):
                out.append(f'{key}: "{val.replace(chr(34), chr(39))}"'); changed.append(key); continue
        out.append(ln)
    return "\n".join(out), changed

for f in ALL:
    if is_frozen(f):  # legacy field rename is safe even on some structural; but skip frozen
        continue
    s = open(f, encoding="utf-8", errors="replace").read()
    head, _, body = split_fm(s)

    # 1) missing frontmatter -> minimal stub (content pages only)
    if head is None and not skip(f) and rel(f).startswith(("domains/", "system/", "concepts/", "entities/", "syntheses/")) \
       or (head is None and os.path.basename(f) in ("plan.md", "log.md", "journal.md") and rel(f).startswith("projects/")):
        ...
        for ln in s.splitlines():
            ...
            if not t or t.startswith(("#", "---", "<!--", ">", "|", "*", "-", "_")):
                continue
            if re.match(r"^\*\*(last updated|phase|status|updated|generated|source)", t, re.I):
                continue
            if t.lower().startswith(("source:", "fetched:", "last updated", "updated:", "generated")):
                continue
            if len(t) < 25:
                continue
            firstline = t; break
        ...
        if safe_write(f, stub + s):
            log("add-frontmatter", f, "stubbed name/title/summary/updatedAt/status (review summary)")
        continue
    ...
    if os.path.basename(f) == "STATUS.md" and re.search(r"^updated:", new_head, re.M) and not re.search(r"^updatedAt:", new_head, re.M):
