...
Three peer services, each restartable without affecting the others:

| Service | Job | Failure mode |
|---|---|---|
| OpenClaw gateway | All I/O — Telegram, voice, cron, conversational agent | Inbound stops; data layer untouched. |
| `lifekit-curator` | Drains `queue.jsonl` → updates domain files | Queue grows; nothing else affected. |
| `swarm` (optional, not in v0.x) | Autonomous build engine | Builds offline; conversational stack unaffected. |
